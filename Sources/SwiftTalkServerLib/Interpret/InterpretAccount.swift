...
extension Route.Account {
    ...
    private func interpret<I: ResponseRequiringEnvironment>(session sess: Session) throws -> I where I.Env == STRequestEnvironment {
        ...
        case .logout:
            return .query(sess.user.deleteSession(sess.sessionId)) {
                return .redirect(to: .home)
            }
            
        case let .register(couponCode, team):
            return .form(registerForm(couponCode: couponCode, team: team), initial: ProfileFormData(sess.user.data), convert: { profile in
                var u = sess.user
                ...
                return .query(user.update()) {
                    if sess.premiumAccess {
                        ...
                        return .redirect(to: .subscription(.new(couponCode: couponCode, team: team)))
                    }
                }
            })
        ...
        case .profile:
            ...
            let f = accountForm()
            return .form(f, initial: data, validate: { _ in [] }, onPost: { result in
                ...
                let errors = u.data.validate()
                if errors.isEmpty {
                    return .onSuccess(promise: recurly.updateAccount(accountCode: u.id, email: u.data.email).promise, do: { _ in
                        .query(u.update()) { _ in
                            .redirect(to: .account(.profile))
                        }
                    }, else: {
                        .write(html: f.render(data, [(field: "", message: "An error occurred while updating your account profile. Please try again later.")]))
                    })
                } else {
                    return .write(html: f.render(result, errors))
                }
            })
